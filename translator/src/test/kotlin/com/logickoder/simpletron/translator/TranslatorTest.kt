...
import com.logickoder.simpletron.translator.utils.extractStatement
    ...
    fun testExtractStatement() {
        for ((index, line) in translator.lines.withIndex()) {
            ...
            if (syntax is Statement) {
                val statement = translator.statements.find { it.keyword == line.split(Regex("\\s+"))[1] }
                assertEquals(statement?.keyword, syntax.keyword)
            }
        }
    }
